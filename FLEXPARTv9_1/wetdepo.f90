...
!hg hz is the model level 
  ...
  !**************************************************************************
!hg lsp =large scale precipitation  
    if (lsp.gt.20.) then
      i=5
...
!hg convp = convective precipitation
    if (convp.gt.20.) then
      j=5
  ...
  ! BELOW CLOUD SCAVENGING  for aerosols and not highliy soluble substances weta=5E-6
  ...
  ! IN CLOUD SCAVENGING
          ...
          if (dquer(ks).gt.0) then ! is particle
           ...
           wetscav=S_i*prec/3.6E6/clouds_h !hg IN CLOUD SCAVENGING
        endif
  ...
  ! Correct deposited mass to the last time step when radioactive decay of gridded deposited mass was calculated
